...
# Author: B. Anderson
# Date: 21 May 2020
# Description: parse tabular (blastn -outfmt 6) BLASTN results
##########################
...
parser = argparse.ArgumentParser(description = 'A script to parse BLASTN results in tabular format (-outfmt 6); use \'6 std qseq sseq qlen slen stitle\' if you wish to have alignments with gaps')


# add arguments to parse
parser.add_argument('blast_out', type=str, help='The blast output to parse')
parser.add_argument('-b', type=str, dest='plot', help='Create (yes or no [default]) a BED-like format file for plotting with e.g. R package Sushi')
...
with open('hits.txt', 'w') as hits_file:
	...
	for hsp in hsp_list:
		if query_file:
			...
			if found_query:
				if int(hsp[6]) < extension + 1:		# if the query hit is near the start of the fasta
					spaces = extension + 1 - hsp[6]
					nuc_query = (' '*spaces + seq_str[:hsp[6] - 1] + ' ' + seq_str[(hsp[6] - 1):hsp[3]] + ' ' +
							seq_str[hsp[3]:(hsp[3] + extension)])
				else:
					nuc_query = (seq_str[(hsp[6] - extension - 1):(hsp[6] - 1)] + ' ' + seq_str[(hsp[6] - 1):hsp[3]] +
							' ' + seq_str[hsp[3]:(hsp[3] + extension)])
		...
		# append the sequences and information to the hit list
		# hsp is 'query', 'sbjct', 'pident', 'length', 'mismatch', 'gapopen', 'query_start', 'query_end', 'sbjct_start', 'sbjct_end', 'evalue', 'bitsore', 'qseq', 'sseq', 'qlen', 'slen', 'stitle'
