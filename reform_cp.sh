...
#	Oct 2023 (clean up and make more useable)
...
merge_script=~/scripts/fasta_merging.py
...
help()
{
(	echo -e "\n\tThis script will reform a single chloroplast fasta file to set consistent \
		start position and orientation of LSC and SSC"
...
# a: find largest possible IR (start may be in an IR) by setting a new contig at
# half length location and comparing blast results
python3 "$reform_script" temp_cp.fasta $(echo "$len / 2" | bc) && mv new_contig.fasta temp_cp2.fasta
...
if [ "$new_start" -ne 1 ]; then
	python3 "$reform_script" temp_cp.fasta "$new_start" && mv new_contig.fasta temp_cp.fasta
fi
...
python3 "$extract_script" -c 1..$(echo "$ira_start - 1" | bc) temp_cp.fasta && mv extract.fasta LSC.fasta
...
python3 "$extract_script" -c $(echo "$ira_end + 1" | bc)..$(echo "$irb_start - 1" | bc) temp_cp.fasta && mv extract.fasta SSC.fasta
...
python3 "$extract_script" -c "$ira_start".."$ira_end" temp_cp.fasta && mv extract.fasta IRA.fasta
python3 "$extract_script" -c "$irb_start".."$irb_end" temp_cp.fasta && mv extract.fasta IRB.fasta

cat LSC.fasta IRA.fasta SSC.fasta IRB.fasta > new_cp.fasta

python3 "$merge_script" new_cp.fasta && mv merged.fasta new_cp.fasta
